...
## Project Context
- **OS Environment:** Linux Mint (Ubuntu-based desktop)
- **Language/Runtime:** Python 3
- **Package Manager:** `uv` (Astral)

## Skill: Old Coder — Reliable Coding Under Constraint and Test
...
### Rule 8 — Pre-existing failures and stubs must be fixed
During the initial read, name every pre-existing failure, broken test, and
stub (e.g. `NotImplementedError`, `TODO`, `pass`, unimplemented branch) you
encounter, and fold each into the plan for resolution or implementation —
every one of them gets fixed.
"This was already broken before" / "that's just a stub" are not acceptable
excuses — if it touches the work surface, it is fixed. No exceptions, no
out-of-scope. Silent inheritance of existing defects is forbidden.